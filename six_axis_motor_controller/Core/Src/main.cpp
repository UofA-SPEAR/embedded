...
static void MX_GPIO_Init(void)
{
    ...
    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOA, CSN_MOTOR_4_Pin | CSN_MOTOR_3_Pin | CSN_MOTOR_2_Pin | CSN_MOTOR_1_Pin | CSN_EEPROM_Pin, GPIO_PIN_SET);
...
{ /* Disables the motors if the board temperature is too high. */
    ...
    if (temp > TEMP_RANGE_FAN && temp < TEMP_RANGE_MAX) {
        ...
        {
            if (motors[i]->CAN_MotorID == CAN_FAN_ID) {
                motors[i]->setTargetSpeed(100);
            }
        }
    }
}
