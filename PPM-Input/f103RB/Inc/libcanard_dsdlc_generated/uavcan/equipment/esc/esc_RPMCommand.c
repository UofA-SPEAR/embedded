...
uint32_t uavcan_equipment_esc_RPMCommand_encode_internal(uavcan_equipment_esc_RPMCommand* source, void* msg_buf, uint32_t offset, uint8_t root_item)
{
    ...
    {
        canardEncodeScalar(msg_buf, offset, 18, (void*)(source->rpm.data + c));// 131071
        offset += 18;
    }
...
int32_t uavcan_equipment_esc_RPMCommand_decode_internal(const CanardRxTransfer* transfer, uint16_t payload_len, uavcan_equipment_esc_RPMCommand* dest, uint8_t** dyn_arr_buf, int32_t offset, uint8_t tao)
{
    ...
    {
        // - Array length 5 bits
        ret = canardDecodeScalar(transfer, offset, 5, false, (void*)&dest->rpm.len); // 131071
        if (ret != 5)
        {
    ...
    for (c = 0; c < dest->rpm.len; c++)
    {
        if (dyn_arr_buf)
        {
            ret = canardDecodeScalar(transfer, offset, 18, true, (void*)*dyn_arr_buf); // 131071
            if (ret != 18)
            {
                goto uavcan_equipment_esc_RPMCommand_error_exit;
            }
