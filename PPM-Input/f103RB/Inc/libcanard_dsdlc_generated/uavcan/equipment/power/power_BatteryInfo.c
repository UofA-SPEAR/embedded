...
uint32_t uavcan_equipment_power_BatteryInfo_encode_internal(uavcan_equipment_power_BatteryInfo* source, void* msg_buf, uint32_t offset, uint8_t root_item)
{
    ...
    canardEncodeScalar(msg_buf, offset, 16, (void*)&tmp_float); // 32767
    offset += 16;
    source->status_flags = CANARD_INTERNAL_SATURATE(source->status_flags, 2047)
    canardEncodeScalar(msg_buf, offset, 11, (void*)&source->status_flags); // 2047
    offset += 11;

    source->state_of_health_pct = CANARD_INTERNAL_SATURATE(source->state_of_health_pct, 127)
    canardEncodeScalar(msg_buf, offset, 7, (void*)&source->state_of_health_pct); // 127
    offset += 7;

    source->state_of_charge_pct = CANARD_INTERNAL_SATURATE(source->state_of_charge_pct, 127)
    canardEncodeScalar(msg_buf, offset, 7, (void*)&source->state_of_charge_pct); // 127
    offset += 7;

    source->state_of_charge_pct_stdev = CANARD_INTERNAL_SATURATE(source->state_of_charge_pct_stdev, 127)
    ...
    {
        canardEncodeScalar(msg_buf, offset, 8, (void*)(source->model_name.data + c));// 255
        offset += 8;
    }
...
int32_t uavcan_equipment_power_BatteryInfo_decode_internal(const CanardRxTransfer* transfer, uint16_t payload_len, uavcan_equipment_power_BatteryInfo* dest, uint8_t** dyn_arr_buf, int32_t offset, uint8_t tao)
{
    ...
    {
        // - Array length 5 bits
        ret = canardDecodeScalar(transfer, offset, 5, false, (void*)&dest->model_name.len); // 255
        if (ret != 5)
        {
            goto uavcan_equipment_power_BatteryInfo_error_exit;
        }
